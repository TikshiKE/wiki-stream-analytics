...
Prerequisites: GitHub repo `TikshiKE/wiki-stream-analytics`, Stage 7 code merged to `main`, Railway account (Hobby plan).
...
Runs migrations on start (entrypoint). Set:
...
POSTGRES_PORT=${{postgres.PGPORT}}
REDIS_URL=${{redis.REDIS_URL}}
DASHBOARD_URL=http://dashboard.railway.internal:8501
HEALTH_PORT=8090
```
...
Merge Stage 7 → GitHub Actions runs `ci.yml` + `build-deploy.yml`.
