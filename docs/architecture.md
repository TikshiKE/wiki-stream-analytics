...
**Trade-off:** partition management overhead; **benefit:** cheap retention, predictable query plans, interview-friendly ops story.
...
| **Kafka disk on Railway** | Volume may be lost on major redeploy | Acceptable for portfolio: live stream refills raw; document in interviews |
...
**Estimated monthly cost (Railway Hobby):** ~**$36–58** depending on uptime and egress — Hobby includes $5 subscription + usage-based compute/storage. Kafka + Airflow are the largest consumers; dbt-docs adds ~$1–2. Tune down if needed (shorter retention, smaller Airflow instance).
